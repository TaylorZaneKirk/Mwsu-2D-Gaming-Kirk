<?php
    ...
    if($serviceid == 0){ //Is custom?
        ...
        if($preparedQuery2 = $conn->prepare('SELECT serviceid FROM nimbusioservices WHERE servicename=? AND servicedesc=?')){
            $preparedQuery2->bind_param('ss', $servicename, $servicedesc);
